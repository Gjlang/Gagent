...
<?php
  ...
  if ($isFullAuditReport && $overallAuditScore !== null) {
    $frictionScore = round(
        (((float) $overallAuditScore - 1) / 2) * 100
    );
...
$comparisonProjectId = request('comparison_project');
$comparisonBeforeRunId = request('compare_from');
$comparisonAfterRunId = request('compare_to');

$hasComparisonResult =
    $comparisonProjectId
    && $comparisonBeforeRunId
    && $comparisonAfterRunId
    && $run
    && (int) $comparisonAfterRunId === (int) $run->id;
...
<div class="g-page-header">
    ...
    <a
        class="g-btn"
        href="<?php echo e(route('reports.download.pdf', $report)); ?>"
    >
        Download PDF
    </a>

    <?php if($hasComparisonResult): ?>
        <a
            class="g-btn g-btn-primary"
            href="<?php echo e(route(
                    'projects.comparison',
                    [
                        'project' => $comparisonProjectId,
                        'before_run' => $comparisonBeforeRunId,
                        'after_run' => $comparisonAfterRunId,
                    ]
                )); ?>"
        >
            Show Comparison
    ...
    <?php elseif(
        ...
        <a
            ...
            href="<?php echo e(route(
                    ...
                    [
                        'comparison_project' => $project->id,
                        'compare_from' => $run->id,
                    ]
                )); ?>"
        >
            Retest Website and Compare
        </a>
    <?php endif; ?>
</div>
</div>
    ...
    <div class="g-split-row">
        ...
        >
            ...
            <button
                ...
                type="submit"
            >
                <?php echo e($report->llm_generated_at
                ...
                >
                    <?php $__empty_1 = true; $__currentLoopData = (
                            $report
                                ->llm_recommendations
                            ?? []
                        ); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $recommendation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <li>
